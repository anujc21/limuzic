...
const SongCard: React.FC<SongCardProps> = ({ song, isPlaying, isCurrent, isLiked, onToggleLike, onClick }) => {
    return (
        <div 
            className={`group relative flex flex-col p-4 rounded-2xl transition-all duration-300 cursor-pointer
                ${isCurrent ? 'bg-white/10 ring-1 ring-white/20 shadow-xl' : 'hover:bg-white/5 hover:shadow-lg'}
            `}
            onClick={onClick}
        >
            ...
            <div className="relative aspect-square w-full rounded-xl overflow-hidden shadow-lg mb-4">
                ...
                <img 
                    src={song.coverUrl} 
                    alt={song.title} 
                    className={`relative z-10 w-full h-full object-contain transition-transform duration-500 ${isCurrent ? 'scale-105' : 'group-hover:scale-105'}`}
                />
            ...
            <div className="flex flex-col gap-1">
                <h3 className={`font-semibold truncate text-lg ${isCurrent ? 'text-violet-300' : 'text-white'}`}>
                    {song.title}
                </h3>
